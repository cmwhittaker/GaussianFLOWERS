...
aep_b,_  = num_F(U_i,P_i,theta_i,
                 ...
                 turb,
                 RHO=1.225,K=0.025,
                 u_lim=U_LIM,cross_ts=True,ex=True,lcl_Cp=True,avCube=True,var_Ct=False)
#then the flow field
...
def nice_plot2(fig,gs,Uw_js):
    ...
    xs1 = [U_inf1,U_inf2]
    ys1 = Ct_f(xs1)
    ax.scatter(xs1,ys1,marker='x',s=10,color='black')
    'an'
    ax.set(ylabel='$C_t$',xlim=(CP_LOWER,CP_UPPER))
    wav_ct = np.sum(P_i*Ct_f(U_i))
    ax.axhline(y=wav_ct,c='black',ls=':',lw=1)
    'an'
    ann_txt = "$\\overline{C_t}="+f"{wav_ct:.2f}"+"$"
    ax.annotate(ann_txt, xy=(26.5,1.02*wav_ct), ha='right', va='bottom',color='black',xycoords='data',rotation='horizontal')
    xs2 = np.linspace(CP_LOWER,CP_UPPER,100)
    ys2 = Ct_f(xs2)
    ax.plot(xs2,ys2,lw=1,color='black')
...
br = 20
gs = GridSpec(6, 3, height_ratios=[br,br,1,1,br,1],wspace=0.2,hspace=0.45)
...
cf = nice_composite_plot_v02(fig,gs[1,:],gs[2,:],ff_a.reshape(xx.shape),xx,yy,z2,xt,yt,tpce)

#next is the zoomed in thrust coefficient curve
nice_plot2(fig,gs[4,:],Uw_ja)
