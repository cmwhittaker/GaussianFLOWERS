...
ax.plot(xs,y1,label='$C_t$',color=colors[0],ls=line_styles[0])
ax.plot(xs,y2,label='$C_p$',color=colors[2],ls=line_styles[2])
ax.set(xlabel='Wind Speed / $ms^{-1}$',ylabel = 'Thrust/Power Coefficient',ylim=(None,0.9))

ax.legend(loc="upper right",frameon=False)

# Adding grey boxes to highlight regions
ax.axvspan(3, 10.8, color='grey', alpha=0.15,ec='none')  # Adjust the x-axis values as per the region you want to highlight
ax.axvspan(11.2, 25, color='grey', alpha=0.15,ec='none')  # Adjust the x-axis values as per the region you want to highlight
