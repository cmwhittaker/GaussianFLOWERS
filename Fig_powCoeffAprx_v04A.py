...
def get_wind_roses(no_bins):
    ...
    #I also manually scale the wind roses 
    wr_list = []
    ab = 1
    scale = [ab,ab,ab]
    #scale = [1.4*ab,1.23*ab,1.00*ab]
    
    for i in range(rows): #for each row
        if i == 0: 
            wr = wind_rose(bin_no_bins=no_bins,custom=None,site=2,Cp_f=turb.Cp_f)
            wr.avMagnitude = wr.avMagnitude*scale[i]
        if i == 1: 
...
def num_F(r_jk,theta_i,theta_jk,
          Ct_f,Cp_f,
          A,RHO=1.225,K=0.025,
...
for i in range(rows):
    ...
    for j in range(cols):
        ...
        a_pow,_ = num_F(u_lim=INVLD_R,cross_ts=True,ex=True,lcl_Cp=True,avCube=True,var_Ct=True)
        b_pow,_ = num_F(u_lim=INVLD_R,cross_ts=True,ex=True,lcl_Cp=False,avCube=True,var_Ct=True)
        #then the flow field(the visualisation)
        r_jk,theta_jk = gen_local_grid_v02(layout,plot_points)
        _, a_ff = num_F(u_lim=INVLD_R,cross_ts=True,ex=True,lcl_Cp=True,avCube=True,var_Ct=True)
...
def nice_composite_plot_v02(fig,gs,row,col,Z1,X,Y,Z2,xt,yt,tpce,vmin,vmax):
    ...
    #contourf
    cf_grey = ax.contourf(X,Y,Z1,cmap=cm.gray,levels=8,vmin=vmin,vmax=vmax)
    #scatter plot
    axs.append(ax)
...
def cp_plot():
    #a plot to show the variation of cp
    ax = fig.add_subplot(gs[7,1:])
    xt1 = np.linspace(3,25,72,endpoint=True)
    ax.plot(xt1,turb.Cp_f(xt1),color='black')
    ax.set(xlim=(3, 25))
    ax2 = ax.twinx()
    ax2.plot(wr.avMagnitude,theta_i)

    return None

def ill_cb(cf): #the illustrative colourbar

    cax = fig.add_subplot(gs[8,1:])
    pos = cax.get_position()
    
    a = pos.x0 + (pos.x1-pos.x0)*(vmin-3)/22
    b = pos.width*(vmax-vmin)/(25-3) 
    ...
    fig.colorbar(sm, cax=cax, orientation='horizontal',format='%.3g')
    return None
...
br = 30
gs = GridSpec(9, cols+1, height_ratios=[br,1,br,1,br,1,1,br,1],wspace=0.15,hspace=0.41)
fig = plt.figure(figsize=(7.8,8), dpi=300)

import numpy as np

vmin,vmax = np.min(np.array(a_ff_array)),np.max(np.array(a_ff_array))
axs = []
...
for i in range(rows): 
    ...
    for j in range(cols): #then onto the contours
        ...
        cf = nice_composite_plot_v02(fig,gs,2*i,j+1,a_ff_array[i][j],X,Y,Z2,xt,yt,tpce,vmin,vmax) 

cp_plot()

ill_cb(cf) #'illustrative' colourbar on bottom row
...
cf = ax.contourf(X,Y,a_ff_array[2][2].reshape(X.shape),50,cmap=cm.gray)
fig.colorbar(cf)