...
P_n = 2*Pwr_NC(U_i[0],0)
P_t = P_n

#next check if num_F and ntag are giving the same result
#off = 5
#layout = np.array(((0,0),(0,off),(off,0),(off,off)))
layout = np.array(((0,0),(-0.2,-3),(+0.2,-3)))

# num_F with the following assumptions:
# cnst thrust coeff (Ct_op=3, wav_Ct=wav_Ct),global power coeff (Cp_op=2), 
# neglect cross terms (cross_ts=False), approx. wake deficit (ex=False), 
# neglect cube terms (cube_term=False)
