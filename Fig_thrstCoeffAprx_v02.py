...
for i in range(len(site_list)):
    ...
    actual_Z,_,_  = cubeAv_v4(r_jk,theta_jk,
                       ...
                       K,
                       turb.A)
    fixed_Ct = np.sum(turb.Ct_f(wr.avMagnitude)*wr.frequency)
...
for i in range(len(site_list)): #for each row
    ...
    nice_cartesian_plot(fig,gs[2*i,1],wr.deg_bin_centers,turb.Ct_f(wr.avMagnitude),i,cnst_thrust_coeff)
